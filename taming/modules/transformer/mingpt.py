...
clip=np.load('text_embedding.npy')
clip=torch.from_numpy(clip).cuda()
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size, block_size, n_layer=12, n_head=8, n_embd=256,
        ...
        #self.label = nn.Sequential(nn.Linear(768, config.n_embd),nn.ReLU(),nn.Linear(config.n_embd, config.n_embd),nn.ReLU(),nn.Linear(config.n_embd, config.n_embd))
        self.label = nn.Sequential(nn.Linear(768, config.n_embd),nn.ReLU(),nn.Linear(config.n_embd, config.n_embd))
        self.tok_emb = nn.Embedding(config.vocab_size, config.n_embd)
        self.tok_emb2 = nn.Embedding(config.vocab_size, config.n_embd)
        #self.tok_emb3 = nn.Embedding(config.vocab_size, config.n_embd)
    ...
    def forward(self, idx, embeddings=None, targets=None,f=0):
        ...
        token_embeddings[:,0:90,:]=self.tok_emb2(idx[:,0:90])
        #token_embeddings[:,90:92,:]=self.tok_emb3(idx[:,90:92])
        for i in range(idx.shape[0]):
            for j in range(30):
                if idx[i,3*j]<200:
                    token_embeddings[i,3*j]=self.label(clip[idx[i,3*j]].to(token_embeddings[i,3*j].device))

        #18,347 ——> 18,347,1024
        #4,220 ——> 4,220,1024 why 220?
        #4,221 ——> 4,221,1024
        #...
        # if f==0:
        #     print(3,token_embeddings.shape)
        #     input()
        ...
        #for j in range(29):
        #    position_embeddings=torch.cat((position_embeddings[:, 0:3, :],position_embeddings),1)
