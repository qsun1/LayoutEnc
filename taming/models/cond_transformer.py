...
class Net2NetTransformer(pl.LightningModule):
    ...
    def forward(self, x, c):
        ...
        _, c_indices = self.encode_to_c(c) # batch_size, 92
        if self.training and self.pkeep < 1.0:
            ...
            a_indices = z_indices # batch_size, 256 = 16*16
      
        ipdb.set_trace()
        cz_indices = torch.cat((c_indices, a_indices), dim=1)
        #18,256+92=348 (18,348)
        # target includes all sequence elements (no need to handle first one
        # differently because we are conditioning)
        target = z_indices
        # make the prediction
        logits, _ = self.transformer(cz_indices[:, :-1])
        #cz_indices[:, :-1]: (18,347) delete the last row
    ...
    def sample(self, x, c, steps, temperature=1.0, sample=False, top_k=None,
        ...
        if self.pkeep <= 0.0:
            ...
            #print('sample')
            logits, _ = self.transformer(idx=x,f=1)
            ...
            for k in range(steps):
                ...
                #print('sample')
                logits, _ = self.transformer(idx=x_cond,f=1)
    ...
    def log_images(self, batch, temperature=None, top_k=None, callback=None, lr_interface=False, **kwargs):
        ...
        quant_z, z_indices = self.encode_to_z(x) #4,256,16,16 #4,256
        quant_c, c_indices = self.encode_to_c(c) #4,92 #4,92
