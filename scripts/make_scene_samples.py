...
from make_samples import get_parser, load_model_and_dset
...
def _helper(coord: int, coord_max: int, coord_window: int) :
...
if __name__ == "__main__":
    ...
    for batch_no, batch in p_bar_1:
        ...
        for i, annotations in tqdm(enumerate(batch['annotations']), desc='within_batch', total=data_loader.batch_size):
            ...
            for j in range(1):
                save_image(imgs[j].unsqueeze(0), outdir.joinpath(f'{batch_no:04}_{i:02}_{j:02}.png'), n_row=opt.n_samples_per_layout+1)
